...
for i,snapshot in enumerate([99,78,62,50]):
	...
	subset = np.random.choice(len(ri[rmask]), size=8000, replace=False)
	plt.scatter(f3['rmag'][rmask][subset], ri[rmask][subset], marker='.', alpha=0.08, color='darkred', s=2.0)
	...
	subset = np.random.choice(len(ri[bmask]), size=8000, replace=False)
	plt.scatter(f3['rmag'][bmask][subset], ri[bmask][subset], marker='.', alpha=0.08, color='royalblue', s=2.0)
	...
	if i+1 in [1,3]:
		...
		yticks=False
	plt.yticks([0,0.1,0.2,0.3,0.4],visible=yticks, fontsize=16)

	z = redshifts[snapshot]
	plt.annotate('$z=%1.2f$'%z, fontsize=16, xy=(-23.75, 0.42))


plt.subplots_adjust(bottom=0.14,left=0.14,right=0.95,top=0.95, hspace=0, wspace=0)

plt.savefig('ri_rmag_panels.pdf')
plt.close()
