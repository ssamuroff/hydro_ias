...
class plot2D(plots.MultinestPlots2D):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.colors=[ 'darkmagenta','darkmagenta', 'midnightblue', 'midnightblue']*10
        self.linestyles=['-','--','-',':']*10
        ...
        matplotlib.rcParams['font.family']='serif'
        self.labels= [ r'Fiducial $b_{\rm TA}$ prior, $z=0$',
                      r'Uninformative $b_{\rm TA}$ prior, $z=0$',
                      r'Fiducial $b_{\rm TA}$ prior, $z=1$',
                      r'Uninformative $b_{\rm TA}$ prior, $z=1$',
        ...
        self.axis=[-5.,5,-3,3.5]
        self.fill_list=[False,True,False,True,False,False,False,False,True,True,True]*10
    ...
    def run(self):
        name1="intrinsic_alignment_parameters--bias_ta"
    ...
    def make_2d_plot(self, name1, name2):
      ...
      #      leg.get_frame().set_edgecolor('white') # this will make the edges of the 
        ...
        pylab.xlabel(r"$b_{\rm TA}$", fontsize=22, fontname='serif')
        pylab.tight_layout()
...
#        pylab.errorbar(x0,y0,xerr=dx,yerr=[0.04], markersize=9.0, markerfacecolor='k',color='k', marker='*',markeredgecolor='k', linestyle='none')
      #  pylab.errorbar([4.35341365],[1.80416667], xerr=7.080459e-01, yerr=1.706998e-02,color='pink',marker='o')
        ...
        pylab.axvline(0.0,color="k",lw=1.5, ls="--")
