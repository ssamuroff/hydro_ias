...
class plot1D(plots.MultinestPlots1D):
    contours=[]
    proxies=[]
    def __init__(self, *args, **kwargs):
        ...
        self.labels= [ #r'2pt $(r_\mathrm{p} > 6 h^{-1} \mathrm{Mpc})$',
                       ...
                       r'2pt $(r_\mathrm{p} > 6 h^{-1} \mathrm{Mpc})$'] 
        self.axis=[-0.5,2.8]
    ...
    def make_1d_plot(self, name1):
        ...
        col = self.reduced_col('intrinsic_alignment_parameters--a1')
        n0, x_axis, like = self.smooth_likelihood(col)
        np.savetxt('/home/ssamurof/like_marginalised_cent.dat',np.vstack((x_axis,like)).T)
