...
class plot2D(plots.MultinestPlots2D):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.colors=['midnightblue','darkmagenta', 'k', 'pink','steelblue',"purple", 'k', 'k', 'k', 'k']*10
        self.linestyles=['-','-','-',':']*10
        ...
        self.axis=[-2.,4.5,-4.5,2]
        self.fill_list=[True,True,False,False,False,False,False,False,True,True,True]*10
        ...
        self.alphas=[0.4,0.4,0.4,0.6,0.4,0.4,0.4]*len(self.colors)
        self.imshow=[False]*10
        self.linecolors=[None]*10
        self.opaque_centre=[False]*10
        self.fill_colors=[None,None]*10
        self.plot_points=[(3.41071,-2.64954),(1.66128,0.225309),(1.98858,-0.98183)]
        #self.proxies=[]
    ...
    def make_2d_plot(self, name1, name2):
        ...
        if self.plot_points is not None:
            point_markers=['*','+','x','^']
            #import pdb ; pdb.set_trace()
            p = self.plot_points[self.source.index]
            m = point_markers[self.source.index]
            pylab.plot(p[0], p[1], m, color=color, markeredgecolor=color, markersize=10, markerfacecolor='none')
 ...
 #       pylab.subplots_adjust(top=0.95)
      ...
      #  pylab.errorbar([4.35341365],[1.80416667], xerr=7.080459e-01, yerr=1.706998e-02,color='pink',marker='o')
        ...
        #import pdb ; pdb.set_trace()
        #A1 = y_axis[np.argwhere(like==like.max())[0,0]]
        #A2 = y_axis[np.argwhere(like==like.max())[0,0]]
        #print('A1 = %3.2f'%A0)
